...
import { getActiveDemoDay } from "./types";
import { calculateReadiness, calculateSessionLoad } from "./seed";
...
export function executeDemoTool(current: DemoState, call: DemoToolCall): { state: DemoState; result: DemoToolResult } {
  ...
  if (call.tool === "add_water") {
    ...
    const session = activeDay.sessions.find((candidate) => candidate.id === call.arguments.sessionId);
    ...
    if (call.arguments.effort !== undefined) session.effortRating = requireInteger(call.arguments.effort, "invalid_effort", 1, 10);
    if (call.arguments.actualDurationMinutes !== undefined) {
      session.actualDurationMinutes = requireInteger(call.arguments.actualDurationMinutes, "invalid_duration", 1, 240);
    }
    session.sessionLoad = calculateSessionLoad(session);
    ...
    activeDay.recovery.modalities = unique;
    activeDay.recovery.score = Math.min(100, 60 + unique.length * 10);
